// extract non-host DNA reads from bam files (alignment to host genome)

process BAM_TO_FASTQ_DNA {
	label "process_750GB"
	tag "${sample_id}"
	publishDir "${params.outdir}/raw/DNA", mode: 'copy'
	publishDir "${params.outdir}/raw/non_host_singletons/DNA", mode: 'copy', pattern: '_singleton.fastq.gz'
